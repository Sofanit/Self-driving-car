...
Original file is located at
    https://colab.research.google.com/drive/1xGVvC7DSpjTZvMyUt7NvLYdRUGEJbusO
"""
...
for filename in file_list:
    if fnmatch.fnmatch(filename, pattern):
        ...
        angle = int(filename.split('.')[0].split('_')[2])
...
default_image = "viddeotest1/train_00000_090.png"
...
def nvidia_model():
    ...
    optimizer = tf.keras.optimizers.Adam(lr=1e-3) # lr is learning rate
...
def image_data_generator(image_paths, steering_angles, batch_size, is_training):
    ...
    while True:
        ...
        for i in range(batch_size):
            ...
            value_exist = os.path.exists(image_path)
            #print(value_exist)
            if value_exist:
...
log_dir_root = 'Model/logs/'
#!rm -rf $log_dir_root

# saves the model weights after each epoch if the validation loss decreased
checkpoint_callback = keras.callbacks.ModelCheckpoint(filepath=os.path.join('Model','monday2_check.h5'), verbose=1, save_best_only=True)
...
model.save(os.path.join('Model','monday2.h5'))

date_str = datetime.datetime.now().strftime("%y%m%d_%H%M%S")
history_path = os.path.join('Model','history.pickle')
...
def predict_and_summarize_with_check(X, Y):
    model = load_model('Model/monday2.h5')
    Y_pred = model.predict(X)
    summarize_prediction(Y, Y_pred)
    return Y_pred

def predict_and_summarize(X, Y):
    model = load_model('Model/monday2_check.h5')
    Y_pred = model.predict(X)
    summarize_prediction(Y, Y_pred)
    return Y_pred
...
y_pred = predict_and_summarize_with_check(X_test, y_test)
...
model = tf.keras.models.load_model('Model/monday2_check.h5')
converter = tf.lite.TFLiteConverter.from_keras_model(model)
tflite_model = converter.convert()
open("monday2_check.tflite", "wb").write(tflite_model)
