...
def motor_go(speed):
    ...
    GPIO.output(AIN1,1) #AIN1
    R_Motor.ChangeDutyCycle(speed)
    GPIO.output(BIN2,0)#BIN2
    GPIO.output(BIN1,1) #BIN1
...
def main():
    ...
    while( camera.isOpened() ):
        ...
        save = image[int(height/3):,:,:]
        save = cv2.cvtColor(save, cv2.COLOR_BGR2YUV)
        save = cv2.GaussianBlur(save, (3,3), 0)
        save_image = cv2.resize(save, (200,66))
